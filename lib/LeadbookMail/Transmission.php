...
{
    public function __construct(LeadbookMail $leadbookmail)
    {
        parent::__construct($leadbookmail, 'transmissions');
    }
    ...
    private function formatShorthandRecipients($payload)
    {
        ...
        for ($i = 0; $i < count($payload['recipients']); ++$i) {
            $payload['recipients'][$i] = $this->toAddressObject($payload['recipients'][$i]);
        }
    ...
    {
        $originalAddress = $this->toAddressString($payload['recipients'][0]);
        foreach ($payload[$listName] as $recipient) {
            $recipient = $this->toAddressObject($recipient);
            $recipient['header_to'] = $originalAddress;
